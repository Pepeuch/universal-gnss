...
- model-aware Unicore rover dynamic mode selection with:
  - `MODE ROVER SURVEY MOW` for documented mower-oriented rover models
    `UM960`, `UM980`, `UM982`, and `UB9A0`
  - safe `MODE ROVER` fallback when the model is unknown or the portable layer
    has no documented support
  - build-requirement warnings for models such as `UM980` (`Build7923+`) and
    `UM982` (`Build7650+`) because the portable planner cannot currently
    verify firmware build metadata
...
- when the selected Unicore model documents mower-oriented rover dynamics, the
  portable rover helpers now emit `MODE ROVER SURVEY MOW`; unknown models keep
  `MODE ROVER`, and models with documented build prerequisites surface a
  warning because firmware build metadata is not yet verified by the planner
- Unicore runtime output commands now follow the documented explicit port form
  `MESSAGE COM1 <rate>` or `MESSAGE COM1 ONCHANGED`; the old `LOG ... ONTIME`
  form is no longer generated by the portable Unicore profiles
