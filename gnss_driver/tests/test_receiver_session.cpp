...
void TestExplicitUbloxMode(TestContext& ctx)
{
  ...
  ctx.Expect(metrics.selected_session_kind ==
...
void TestExplicitUnicoreMode(TestContext& ctx)
{
  ...
  const auto& state = session.current_state();
  ctx.Expect(metrics.selected_session_kind ==
                     std::optional<ReceiverSessionKind>(ReceiverSessionKind::kUnicore) &&
                 metrics.selection_locked,
             "explicit Unicore mode should start selected and locked");
  ctx.Expect(state.timestamp_ns == std::optional<std::int64_t>(2000) && state.fix_valid &&
  ...
  ctx.Expect(metrics.runtime_updates == 1u && session.unicore_metrics().records_parsed == 1u &&
...
void TestExplicitNmeaMode(TestContext& ctx)
{
  ...
  session.FeedBytes(
      BuildNmeaSentence("GPGGA,123519,4807.038,N,01131.000,E,4,08,0.9,545.4,M,46.9,M,,"), 2100);
  session.FeedBytes(BuildNmeaSentence("GPGSA,A,3,04,05,09,12,24,25,29,31,,,,,1.8,1.0,1.5"), 2101);
  session.FeedBytes(
      BuildNmeaSentence("GPGSV,2,1,08,01,40,083,41,02,17,308,43,12,25,120,42,14,10,220,39"), 2102);
  session.FeedBytes(BuildNmeaSentence("GPGST,123519.00,1.2,0.8,0.7,45.0,0.5,0.6,1.1"), 2103);
  session.FeedBytes(BuildNmeaSentence("GPVTG,054.7,T,034.4,M,005.5,N,010.2,K,A"), 2104);
...
void TestAutoModeSelectsUblox(TestContext& ctx)
{
  ...
  auto nmea = BuildNmeaSentence("GPGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,");
...
void TestAutoModeSelectsNmeaWhenEnabled(TestContext& ctx)
{
  ...
  session.FeedBytes(
      BuildNmeaSentence("GPGGA,123519,4807.038,N,01131.000,E,1,08,0.9,545.4,M,46.9,M,,"), 4250);
...
void TestFinalizeAndReset(TestContext& ctx)
{
  ...
  const auto& metrics = session.metrics();
  const auto& state = session.current_state();
  ctx.Expect(metrics.bytes_seen == 0u && metrics.runtime_updates == 0u &&
                 metrics.malformed_records == 0u &&
                 metrics.selected_session_kind ==
                     std::optional<ReceiverSessionKind>(ReceiverSessionKind::kUblox) &&
                 metrics.selection_locked,
