...
ReceiverConfigApplication::Start(std::vector<ReceiverCommand> commands,
...
ReceiverConfigApplicationResult
ReceiverConfigApplication::Step(const std::optional<ReceiverCommandTimestampNs> timestamp_ns)
{
  if (state_ == ReceiverConfigApplicationState::kIdle)
  {
    ...
    result.engine_result = MakeApplicationEngineResult(
        ReceiverCommandTransactionEngineStepStatus::kIdle, "configuration application is idle");
    return result;
  }
...
ReceiverConfigApplicationResult
ReceiverConfigApplication::ApplyResponse(const ReceiverCommandResponse& response,
...
ReceiverConfigApplicationResult
ReceiverConfigApplication::MarkTimeout(const std::optional<ReceiverCommandTimestampNs> timestamp_ns)
{
  return HandleTimeoutResult(engine_.MarkTimeout(timestamp_ns), timestamp_ns);
}

ReceiverConfigApplicationResult
ReceiverConfigApplication::CheckTimeout(const ReceiverCommandTimestampNs now_timestamp_ns)
{
