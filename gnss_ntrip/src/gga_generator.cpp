...
bool GgaGenerationResult::ok() const
{
  return error == GgaGenerationError::kNone;
}
...
GgaGenerationResult BuildNmeaGgaSentence(const universal_gnss::GnssRuntimeState& state)
{
  ...
  GgaGenerationResult result;
  result.fix_quality = MapRuntimeStateToGgaFixQuality(state);

  if (!state.latitude_deg.has_value())
  {
    result.error = GgaGenerationError::kMissingLatitude;
    return result;
  }
  if (!state.longitude_deg.has_value())
  {
    result.error = GgaGenerationError::kMissingLongitude;
    return result;
  }
  ...
  {
    result.error = GgaGenerationError::kInvalidLatitude;
    return result;
  }
  ...
  {
    result.error = GgaGenerationError::kInvalidLongitude;
    return result;
  }
