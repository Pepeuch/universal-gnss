...
def discover_container(explicit: str | None) -> dict[str, Any]:
    ...
    for line in result["stdout"].splitlines():
        ...
        if len(fields) != 4:
            continue
        name, compose_service, image_source, image_title = fields
        normalized_source = image_source.rstrip("/").removesuffix(".git").lower()
        if (
            compose_service == "universal-gnss"
            or normalized_source.endswith("/universal-gnss")
            or "universal gnss" in image_title.lower()
        ):
            names.append(name)
    if len(names) != 1:
        return {"ok": False, "error": f"expected one running Universal GNSS container, found {len(names)}"}
    return {"ok": True, "container": names[0], "method": "runtime_labels"}
...
def sample(args: argparse.Namespace, container_name: str | None, index: int) -> dict[str, Any]:
    ...
    if not container_name:
        record.update({
            "container": {"available": False, "error": "container unavailable: Docker observation skipped"},
            "processes": {"available": False, "error": "container unavailable: process observation skipped"},
            "ros": {"available": False, "error": "container unavailable: ROS observation skipped"},
            "resources": {"available": False, "error": "container unavailable: resource observation skipped"},
        })
        return record
