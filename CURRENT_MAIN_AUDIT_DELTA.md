...
Pre-fix reproduction: Focused regressions constructed real RTCM frames with a computed valid CRC, then supplied undersized 1005/1006, MSM, and 1230 payloads. Before the production change the monitor test failed 11 assertions: malformed base+MSM and each mixed valid/malformed pair satisfied correction requirements; malformed MSM refreshed the semantic window; all 49 recognized MSM family/variant types collectively reported `malformed_msm_satisfying_requirements=49`; malformed 1006 satisfied an explicit 1006 requirement; truncated and decoded-but-validity-cleared 1230 satisfied an explicit GLONASS-bias requirement; `parser_healthy` stayed true; and invalid optional 1230 content from another station could clear otherwise healthy station-owned state. A final adversarial extension was also run before its corresponding production changes: the parser failed 8 assertions (all seven MSM variants truncated inside mask-declared data plus a 72-cell mask), and the monitor failed 4 assertions because a CRC-valid foreign-station MSM truncated after its masks was accepted and an invalid foreign 1230 replaced station-owned diagnostic state. After adding the body validator but before adding the observation-eligibility gate, a complete zero-cell MSM still failed 2 monitor assertions by satisfying health and changing ownership. A final ownership-order regression then failed 2 assertions before its fix: foreign invalid 1230/zero-cell MSM decoded before ownership survived establishment of another station.

Semantic validity contract: `ObserveFrame` now separates an integrity-valid frame observation from a semantic acceptance. A raw 1005/1006 is accepted only when `ParseRtcmBaseStationArp` returns a record. `ParseRtcmMsmSummary` now rejects cell masks beyond RTCM's 64-cell capacity and payloads too short for the satellite/cell data declared by MSM variants 1-7; a structurally decoded MSM is accepted for correction health only when at least one observation cell is populated. RTCM 1230 is accepted for health only when `ParseRtcmGlonassCodePhaseBias` returns a record whose existing `valid` flag is true. A raw type without an implemented semantic decoder remains frame-visible but cannot satisfy a semantic requirement. `ObserveMessage` remains the trusted already-classified API and explicitly records both layers.

Production fix and bookkeeping: `RtcmCorrectionMonitor` keeps separate message-type and MSM-constellation activity maps for semantic acceptance. `HasRequiredMessageTypes` and `HasRequiredCorrectionMessages` consume only those semantic maps; 1005/1006 remain static once successfully decoded, while populated MSM/valid 1230 remain subject to the configured dynamic observation window. Frame-level `total_frames`, `valid_frames`, `MessageCount`, per-type/per-constellation rates, and last-seen timestamps still record complete CRC-valid frames, so UGA-005 correction-flow liveness is unchanged. Structurally decoded zero-cell MSM remains visible in decoder counters while leaving `parser_healthy=true` and `correction_available=false`; malformed known payloads increment their existing rejected counters, now make `parser_healthy=false`, and 1005/1006 also emit `rtcm.base_station_arp_malformed`.

Ownership/static/1230 behavior: Station ownership is evaluated only from a semantically accepted station-bearing record. Malformed 1005/1006 cannot establish or steal ownership, replace retained static metadata, or refresh its decoded timestamp. Body-truncated or observation-empty MSM cannot establish ownership or refresh the semantic MSM window; decoded-but-ineligible MSM/1230 from another station does not replace station-owned diagnostic state. If such unowned decoded state precedes ownership, establishing a different station purges it before recording the accepted message. A same-station or ownerless validity-cleared 1230 remains decoded and informational for diagnostics but cannot satisfy an explicit requirement; `ConfigurePortableRtkCorrectionRequirements` still leaves 1230 optional, so UGA-015 is preserved.

Fixture correction: `gnss_ntrip/tests/test_ntrip_client.cpp` had one stream-health fixture that used the old two-byte `BuildRtcmFrame(1005)` while expecting semantic base metadata. After the fix it was the sole real NTRIP failure; it now uses the pre-existing complete `BuildRtcm1005Frame(42)` helper. The fixture had encoded the defect rather than valid product behavior.

Files changed: `gnss_protocols/src/rtcm_parser.cpp`, `gnss_protocols/src/rtcm_correction_monitor.cpp`, its private monitor header, the focused parser/monitor tests, the corrected NTRIP semantic fixture, and this UGA-008 delta entry.

Post-fix validation: The focused parser and monitor regressions pass 2/2, including valid replacements restoring required-message satisfaction and `correction_available`, the 49 MSM rejection sweep, MSM1-7 body truncation, oversized and zero-cell masks, semantic timestamp isolation, unsupported-type isolation, static metadata retention, station ownership, and optional/explicit 1230 cases. Complete `gnss_protocols` passed 18/18, complete `gnss_ntrip` passed 7/7, complete non-ROS CTest passed 61/61, and the normal sequential ROS2 CTest matrix passed 6/6; ReceiverNode passed 43/43 cases and NtripNode 11/11. Combined ASan+UBSan passed the RTCM parser, base-position, and correction-monitor binaries 3/3 with leak detection disabled because LeakSanitizer cannot run under the sandbox tracer.

Regression reconfirmation: NTRIP tests still prove that complete CRC-valid frames—including semantically incomplete frames used only for liveness—advance correction-flow and operational-backoff milestones (UGA-005/006). The full protocol/NTRIP/ROS runs retained future-clock rejection (UGA-014), optional 1230 (UGA-015), static 1005/1006 lifetime (UGA-016), bounded histories (UGA-017), source/station ownership and reconnect behavior (UGA-018), forwarding active/stale recovery (UGA-019), and cadence-aware runtime freshness (UGA-020).

API/ABI and remaining limitation: No ROS message, topic, parameter, or wire layout changed. Semantic requirement results and `HasRequiredMessageTypes` intentionally become stricter. The monitor's private layout changes, so dependent C++ binaries must be rebuilt. Semantic eligibility is limited to the currently implemented specialized decoders; a newly supported required RTCM type must gain a semantic decoder before raw frames of that type can satisfy health. Malformed counters remain session-lifetime diagnostics, so a later valid replacement restores correction availability but `parser_healthy` remains false until the monitor/session is reset, consistent with the existing lifetime treatment of invalid frames.

Recommended next action: Keep the CRC-valid malformed sweep and mixed-stream ownership cases in CI. A live caster/base capture is recommended to confirm operator diagnostics and four-layer status transitions with production cadence, but hardware is not required to validate this deterministic parser/monitor fix. Existing `MOWGLINEXT_TODO.md` already tracks separate transport, correction-flow, and semantic-health consumption plus malformed RTCM operator behavior, so this fix creates no new downstream work item.
...
Current evidence: `GnssRuntimeState` now carries update-only `clear_value_flags` for capability-backed fields and `clear_direct_value_flags` for latitude, longitude, and altitude. `SetOptionalValue` cancels an earlier clear intent, `ClearOptionalValue` records explicit invalidation, and the new `OmitOptionalValue` resets a candidate value without requesting aggregate invalidation. `GnssRuntimeAggregator` applies SET and CLEAR through the same per-field timestamp/version gate: omission does nothing, CLEAR resets the stored optional and advances its version, an older SET is rejected, and a later SET restores the field. The aggregate retains capability support metadata but recomputes current `value_flags`; update-only clear masks never leak into aggregate/public state.

Files changed: `gnss_core/include/universal_gnss/gnss_runtime_state.hpp`, `gnss_core/include/universal_gnss/gnss_runtime_aggregator.hpp`, `gnss_core/tests/test_runtime_aggregator.cpp`, `gnss_protocols/src/nmea_parser.cpp`, `gnss_protocols/src/ubx_parser.cpp`, `gnss_protocols/src/unicore_parser.cpp`, `gnss_protocols/tests/test_ubx_nav_pvt.cpp`, `gnss_protocols/tests/test_unicore_ascii_parser.cpp`, `gnss_driver/src/unicore_session.cpp`, `gnss_driver/tests/test_nmea_session.cpp`, and this delta report.

Regression tests added: `TestExplicitClearIsDistinctFromOmission` covers SET→OMIT, SET→CLEAR, SET→SET, CLEAR→OMIT, SET→CLEAR→SET, unrelated-field preservation, identical-value SET, invalidating provenance, and rejection of an older SET. `TestDirectPositionClearUsesFieldProvenance` applies the same ordering contract to coordinates. `TestExplicitInvalidityClearsRuntimeValues`, `TestInvalidFixClearsPreviouslyMappedValues`, and `TestExplicitInvalidityClearsMappedUnicoreValues` prove that real NMEA, u-blox NAV-PVT, and Unicore PVTSLNA invalidity reaches the shared primitive and clears public availability without clearing unrelated fields or support metadata.

Pre-fix failure evidence: Before the shared fix, `gnss_core_test_runtime_aggregator` failed 6 assertions: CLEAR was not applied, the old optional and availability flag remained, aggregate provenance did not advance, CLEAR→OMIT retained the stale value, and an older SET resurrected/retained it. After the shared primitive was implemented but before protocol mapping changes, the focused family tests still failed 1 NMEA assertion (cached coordinates), 4 u-blox assertions (coordinates, accuracy, heading, availability), and 3 Unicore assertions (coordinates, accuracy, baseline geometry). This separated the core merge defect from missing protocol invalidation signals.

Production fix and protocol/session implications: NMEA GGA/RMC/GSA no-fix paths clear current position; the existing GSA/GSV/GST explicit DOP, satellite, CN0, and accuracy clears now propagate. u-blox NAV-PVT/NAV-STATUS invalid position state clears coordinates and associated accuracy/heading availability; an invalid heading-validity bit clears heading only. Unicore known no-fix state clears coordinates and position accuracy, while a known unsolved PVTSLN baseline clears cached geometry/compatibility heading but retains the current unsolved status. Unknown or absent protocol fields remain omission. Unicore mixed-NMEA/richer-record pruning was explicitly converted to `OmitOptionalValue`, preserving its existing fallback-suppression behavior rather than turning it into CLEAR.

Post-fix validation: Focused UGA-021 CTest passed 4/4. Complete `gnss_core` passed 3/3, `gnss_driver` passed 19/19, `gnss_protocols` passed 18/18, and complete non-ROS CTest passed 61/61. ROS2 projection remained green: ReceiverNode passed 1/1 CTest (43/43 gtests), NtripNode passed 1/1 (11/11 gtests), and the remaining four ROS2 CTest targets passed 4/4 sequentially. This reconfirms UGA-002/007/014/020 temporal behavior in ReceiverNode and the NtripNode cached-observation behavior. A parallel ReceiverNode/NtripNode run caused the pre-existing wall-clock-sensitive Ntrip test to miss its timing window; both binaries pass serially, which is the normal CTest mode.

Sanitizer result: The six touched non-ROS binaries passed 6/6 under combined ASan+UBSan with no diagnostic. LeakSanitizer alone was disabled (`ASAN_OPTIONS=detect_leaks=0`) because it terminates under the supervised/ptraced execution environment before running tests.

Public/API impact and remaining limitation: This is an additive C++ source API change but changes `GnssRuntimeState` layout/ABI, so dependent C++ binaries must rebuild. Capability flags intentionally remain support metadata after CLEAR; current availability is expressed by cleared optionals/value flags. No ROS message definition, topic, parameter, timestamp-domain architecture, or wire ABI changed, and adapters project cleared values as unavailable/NaN or suppress `NavSatFix` as before. Only explicit known invalidity clears state; ordinary unsupported/missing fields remain NO UPDATE. No new MowgliNext integration contract was created.

Remaining defect, if any: None for UGA-021's omitted-versus-explicit-clear ambiguity in the mapped NMEA, u-blox, and Unicore runtime paths.

Recommended next action: Keep the shared ordering/provenance regressions and the three family-specific invalidation tests in their normal component lanes.
