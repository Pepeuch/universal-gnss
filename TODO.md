...
- [ ] published multi-architecture image indexes with attached SBOM and provenance attestations (the tag-gated Kilted/Lyrical amd64+arm64 workflow is implemented; registry artifacts remain release-action evidence)
...
- [x] secret-file / environment-variable policy with no credential leakage in
  logs or diagnostics
- [x] persistent diagnostic/log/export directory
...
- [x] structured logs suitable for Docker/Compose/BlueOS collection
...
- [x] Docker DNS/reconnect behavior validation
...
- **REQUIRES_IMAGE_PUBLICATION:** publish the Kilted and Lyrical amd64+arm64
  image indexes from an immutable release tag and verify their registry digests
  and attached SBOM/provenance attestations. Publication is not authorized for
  this batch and receives no credit from the implemented workflow alone.
...
- **Software closeout complete (2026-09-06):** no open v0.7 gate has an unfinished
  ...
  long-duration, or registry publication evidence. The Docker quick-start,
  production deployment, serial/device permissions, and ROS2 integration
  guidance are complete in [`docs/ros2_docker.md`](docs/ros2_docker.md).
...
- **Native-arm64 proven (2026-09-05):** native Kilted build/runtime, bounded
  ...
  coverage.
- **PROVEN — external-LAN:** physical robot/second-RPi discovery and distinct
  nonce payloads passed in both directions, followed by different-domain zero
  discovery and zero payload. Unmodified default-bridge multicast failed; the
  sole minimal alternative retained bridge mode and used explicit external
  unicast locators with UDP 36662/36663. Host networking was not used, domain
  IDs are not a security boundary, and this separate matrix receives no release
  checkbox credit.

Phase-E reconciliation (2026-09-05): clean container stop/start, unexpected
main launch-process exit with `unless-stopped` recovery, Docker daemon restart,
and host reboot/autostart all restored the exact external configuration and
fresh healthy u-blox runtime under the sole by-id grant. No operator receiver
provisioning replay was needed. An isolated receiver-node SIGKILL instead left
the container running but unhealthy with restart count 0, and operator
`docker kill` exited 137/OOM false but remained stopped with restart count 0;
those failures remain explicit and are not substituted for crash/reboot PASS
evidence. No USB hotplug/incarnation or receiver persistence is inferred.
...
- [x] TLS support, including bounded/cancellable NTRIP handshake setup
