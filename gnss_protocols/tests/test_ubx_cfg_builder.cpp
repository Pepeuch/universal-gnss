...
{
  return static_cast<std::uint16_t>(frame[4u]) | (static_cast<std::uint16_t>(frame[5u]) << 8u);
}
...
bool FrameChecksumMatches(const std::vector<std::uint8_t>& frame)
{
  ...
  const auto checksum =
      universal_gnss_protocols::ComputeUbxChecksum(frame.data() + 2u, frame.size() - 4u);
  return checksum.ck_a == frame[frame.size() - 2u] && checksum.ck_b == frame[frame.size() - 1u];
}
...
void TestValsetFrameGeneration(TestContext& ctx)
{
  ...
  ctx.Expect(result.payload.size() == 12u && result.payload[0u] == 0x01u &&
                 result.payload[1u] == 0x03u && result.payload[2u] == 0x00u &&
                 result.payload[3u] == 0x00u,
             "VALSET payload should use version 1 and pack layer bits correctly");
  ctx.Expect(result.payload[4u] == 0x01u && result.payload[5u] == 0x00u &&
                 result.payload[6u] == 0x52u && result.payload[7u] == 0x40u,
             "VALSET should pack keys in little-endian order");
  ctx.Expect(result.payload[8u] == 0x00u && result.payload[9u] == 0x10u &&
                 result.payload[10u] == 0x0Eu && result.payload[11u] == 0x00u,
             "VALSET should pack U4 values in little-endian order");
  ctx.Expect(result.frame.size() == 20u && result.frame[0u] == 0xB5u && result.frame[1u] == 0x62u &&
                 result.frame[2u] == 0x06u && result.frame[3u] == 0x8Au &&
...
void TestValgetFrameGeneration(TestContext& ctx)
{
  const auto result = universal_gnss_protocols::BuildUbxCfgValgetFrame(
      UbxCfgLayer::kRam, std::vector<std::uint32_t>{kUart1Baudrate, kMsgoutUbxNavPvtUart1}, 5u);

  ctx.Expect(result.status == UbxCfgBuilderStatus::kOk, "VALGET should build successfully");
  ctx.Expect(result.payload.size() == 12u && result.payload[0u] == 0x00u &&
                 result.payload[1u] == 0x00u && result.payload[2u] == 0x05u &&
                 result.payload[3u] == 0x00u,
             "VALGET should use request version 0 and pack the position field");
  ctx.Expect(result.payload[4u] == 0x01u && result.payload[5u] == 0x00u &&
                 result.payload[6u] == 0x52u && result.payload[7u] == 0x40u &&
                 result.payload[8u] == 0x07u && result.payload[9u] == 0x00u &&
                 result.payload[10u] == 0x91u && result.payload[11u] == 0x20u,
             "VALGET should pack multiple keys in little-endian order");
  ctx.Expect(result.frame[2u] == 0x06u && result.frame[3u] == 0x8Bu &&
                 FrameChecksumMatches(result.frame),
...
void TestMultipleKeyValuePacking(TestContext& ctx)
{
  ...
  ctx.Expect(result.payload[14u] == 0x01u, "U1 values should be packed as one byte");
...
void TestHelperBuilders(TestContext& ctx)
{
  ...
  ctx.Expect(enable_nav_pvt.status == UbxCfgBuilderStatus::kOk &&
                 enable_nav_pvt.payload[4u] == 0x07u && enable_nav_pvt.payload.back() == 0x01u,
             "enable-message helper should target the requested output key");

  const auto disable_gga = universal_gnss_protocols::BuildDisableMessageFrame(kMsgoutNmeaGgaUart1);
  ctx.Expect(disable_gga.status == UbxCfgBuilderStatus::kOk && disable_gga.payload.back() == 0x00u,
  ...
  ctx.Expect(baud.status == UbxCfgBuilderStatus::kOk && baud.payload[8u] == 0x00u &&
                 baud.payload[9u] == 0x08u && baud.payload[10u] == 0x07u &&
  ...
  ctx.Expect(uart2_baud.status == UbxCfgBuilderStatus::kOk && uart2_baud.payload[4u] == 0x01u &&
                 uart2_baud.payload[5u] == 0x00u && uart2_baud.payload[6u] == 0x53u &&
                 uart2_baud.payload[7u] == 0x40u && uart2_baud.payload[8u] == 0x00u &&
                 uart2_baud.payload[9u] == 0x08u && uart2_baud.payload[10u] == 0x07u &&
  ...
  ctx.Expect(rate_hz.status == UbxCfgBuilderStatus::kOk && rate_hz.payload[4u] == 0x01u &&
                 rate_hz.payload[5u] == 0x00u && rate_hz.payload[6u] == 0x21u &&
                 rate_hz.payload[7u] == 0x30u && rate_hz.payload[8u] == 100u &&
  ...
  ctx.Expect(enable_galileo.status == UbxCfgBuilderStatus::kOk &&
                 enable_galileo.payload[4u] == 0x21u && enable_galileo.payload[5u] == 0x00u &&
                 enable_galileo.payload[6u] == 0x31u && enable_galileo.payload[7u] == 0x10u &&
...
void TestRejectedInputs(TestContext& ctx)
{
  const auto bad_size = universal_gnss_protocols::BuildUbxCfgValsetFrame(
      {UbxCfgLayer::kRam}, UbxCfgKeyValue{kUart1Baudrate, UbxCfgValue::U1(1u)});
  ...
  const auto bad_uart2_size = universal_gnss_protocols::BuildUbxCfgValsetFrame(
      {UbxCfgLayer::kRam}, UbxCfgKeyValue{kUart2Baudrate, UbxCfgValue::U1(1u)});
  ...
  const auto no_keys = universal_gnss_protocols::BuildUbxCfgValgetFrame(
      UbxCfgLayer::kRam, std::vector<std::uint32_t>{});
