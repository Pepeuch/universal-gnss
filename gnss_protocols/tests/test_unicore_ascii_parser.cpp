...
void TestPvtslnParsingAndRuntimeMapping(TestContext& ctx)
{
  ...
  ctx.Expect(record.header.timestamp_ns == 1111 && record.header.gps_week == 2190u &&
                 record.header.gps_millis_of_week == 364536000u,
             "PVTSLNA should preserve the framing timestamp and parse the ASCII header");
  ctx.Expect(record.best_tracked_satellites == 46u && record.best_used_satellites == 28u &&
                 record.hdop.has_value() && *record.hdop == 0.6840f,
  ...
  ctx.Expect(state.latitude_deg == 40.07898130522 && state.longitude_deg == 116.23663134427 &&
  ...
  ctx.Expect(
      universal_gnss::HasValueAvailable(state, GnssCapability::kHorizontalAccuracy) &&
          ...
          universal_gnss::HasValueAvailable(state, GnssCapability::kBaselineSolutionStatus),
...
void TestBestNavCorrectionStateSemantics(TestContext& ctx)
{
  ...
  {
    const auto state =
        universal_gnss_protocols::UnicoreBestNavToRuntimeState(*single_result.record);
...
void TestRtkStatusAndRtcmStatusParsing(TestContext& ctx)
{
  ...
  {
    const auto state =
        universal_gnss_protocols::UnicoreRtkStatusToRuntimeState(*rtk_fixed_result.record);
    ctx.Expect(state.fix_valid && state.fix_type == GnssFixType::kRtkFixed &&
                   state.rtk_mode == GnssRtkMode::kFixed,
    ...
    ctx.Expect(
        universal_gnss::HasCapability(state, GnssCapability::kDualAntennaBaseline) &&
            ...
            state.baseline_solution_status ==
                std::optional<GnssBaselineSolutionStatus>(GnssBaselineSolutionStatus::kComputed),
  ...
  const std::string rtk_float_line =
      WithUnicoreAsciiCrc("#RTKSTATUSA,97,GPS,FINE,2190,365355000,0,0,18,1;"
  ...
  {
    const auto state =
        universal_gnss_protocols::UnicoreRtkStatusToRuntimeState(*rtk_float_result.record);
    ctx.Expect(state.fix_valid && state.fix_type == GnssFixType::kRtkFloat &&
                   state.rtk_mode == GnssRtkMode::kFloat && state.dual_antenna_heading == false,
  ...
  if (rtcm_result.record.has_value())
  {
    ...
    ctx.Expect(record.message_type == 1124u && record.message_count == 21186u &&
                   record.satellites_in_message == 21u && record.l3_observables == 11u,
...
{
  const std::string line = WithUnicoreAsciiCrc("#SATSINFOA,96,GPS,FINE,2215,367199000,0,0,18,16;"
                                               "3,2,0,0,0,63,"
  ...
  const auto& record = *result.record;
  ctx.Expect(record.header.timestamp_ns == 5555 && record.tracked_satellite_count == 3u &&
                 record.parsed_satellite_count == 3u && record.frequency_flag == 63u,
             "SATSINFOA should parse the common header and tracked-satellite count");
  ctx.Expect(record.satellites[0].satellite_id == 2u && record.satellites[0].azimuth_deg == 302 &&
                 record.satellites[0].elevation_deg == 51 &&
                 record.satellites[0].cn0_db_hz == 45u &&
                 record.satellites[1].frequency_count == 3u &&
                 record.satellites[1].cn0_db_hz == 43u && record.satellites[2].system_id == 1u,
  ...
  ctx.Expect(universal_gnss::HasValueAvailable(state, GnssCapability::kSatellitesTracked) &&
                 ...
                 state.mean_cn0_db_hz == 46.0f && state.max_cn0_db_hz == 50.0f,
...
{
  const std::string line = WithUnicoreAsciiCrc("#BESTSATA,79,GPS,FINE,2203,226245800,0,0,18,22;"
                                               "4,GPS,2,GOOD,00000013,GLONASS,2-4,GOOD,00000010,"
                                               "GALILEO,5,GOOD,00000001,BEIDOU,20,GOOD,00000000");
  ...
  const auto& record = *result.record;
  ctx.Expect(record.header.timestamp_ns == 5656 && record.entry_count == 4u &&
  ...
  ctx.Expect(record.satellites[0].constellation ==
                 ...
                 record.satellites[0].used_in_solution && record.satellites[0].common_view,
  ...
  ctx.Expect(record.satellites[1].constellation ==
                 ...
                 record.satellites[1].glonass_frequency_channel ==
                     std::optional<std::int16_t>(-4) &&
                 !record.satellites[1].used_in_solution && record.satellites[1].common_view,
  ...
  ctx.Expect(record.satellites[2].constellation ==
                 ...
                 record.satellites[2].used_in_solution && !record.satellites[2].common_view &&
  ...
  ctx.Expect(universal_gnss::HasValueAvailable(state, GnssCapability::kSatellitesTracked) &&
                 ...
                 state.satellites_tracked == 4u && state.satellites_used == 2u,
...
void TestRfAndHardwareParsingAndMapping(TestContext& ctx)
{
  ...
  if (jam_result.record.has_value())
  {
    ...
    ctx.Expect(record.position_type == universal_gnss_protocols::UnicorePositionType::kSingle &&
                   ...
                   record.cw_state == universal_gnss_protocols::UnicoreJammingState::kStrongJamming,
  ...
  if (freq_jam_result.record.has_value())
  {
    ...
    ctx.Expect(record.l1.cw_ratio == 255u &&
                   ...
                   record.l2.cw_state == universal_gnss_protocols::UnicoreJammingState::kNone &&
                   record.l5.cw_state == universal_gnss_protocols::UnicoreJammingState::kJamming,
  ...
  const std::string hw_status_line =
      WithUnicoreAsciiCrc("#HWSTATUSA,97,GPS,FINE,2221,111183000,0,0,18,15;66807,0.920,1.020,0.908,"
                          "0,0.693,0.0,0x00,0,0x0377,0,0");
  ...
  if (hw_status_result.record.has_value())
  {
    ...
    ctx.Expect(record.dc09_v == 0.92f && record.dc10_v == 1.02f && record.dc18_v == 0.908f &&
                   !record.clock_drift_valid && record.hw_flag == 0x00u &&
  ...
  const std::string agc_line =
      WithUnicoreAsciiCrc("#AGCA,65,GPS,FINE,2190,375570000,0,0,18,37;44,46,63,-1,-1,41,1,0,-1,-1");
  const auto agc_result =
      universal_gnss_protocols::ParseUnicoreAgc(BuildAsciiFrame(agc_line, 7780));
...
void TestMalformedAndMissingFields(TestContext& ctx)
{
  ...
  {
    const auto state =
        universal_gnss_protocols::UnicorePvtslnToRuntimeState(*partial_result.record);
  ...
  const std::string empty_satsinfo =
      WithUnicoreAsciiCrc("#SATSINFOA,96,GPS,FINE,2215,367199000,0,0,18,16;0,2,0,0,0,63");
...
void TestExplicitInvalidityClearsMappedUnicoreValues(TestContext& ctx)
{
  ...
  valid.baseline_solution_status = universal_gnss_protocols::UnicoreSolutionStatus::kSolComputed;
  ...
  ctx.Expect(
      state.satellites_used == std::optional<std::uint16_t>(28u) &&
