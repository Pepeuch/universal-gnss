...
int main(int argc, char** argv)
{
  ...
  const auto result = universal_gnss_tools::InspectGnssStreamStream(*input, !summary_only);
  if (input->bad())
  {
  ...
  if (json_output)
  {
    std::cout << universal_gnss_tools::FormatGnssStreamInspectionJson(result, summary_only);
  }
  else
  {
    std::cout << universal_gnss_tools::FormatGnssStreamInspectionText(result, summary_only);
  }
